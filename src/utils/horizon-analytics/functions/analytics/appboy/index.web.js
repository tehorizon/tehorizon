...
const registerUserAppBoy = (userProfile) => {
  ...
  appboy
    .getUser()
    .setGender(userProfile?.gender === "male" || appboy.User.Genders.FEMALE);
  appboy.getUser().setPhoneNumber(userProfile?.mobile_phone ?? "9713412321");

  try {
    const dob = moment(userProfile.date_of_birth, "DD/MM/YYYY");
    appboy.getUser().setDateOfBirth(dob.year(), dob.month() + 1, dob.date());
  } catch (error) {
    console.log({ appBoyDateSetError: error });
  }
  appboy
    .getUser()
