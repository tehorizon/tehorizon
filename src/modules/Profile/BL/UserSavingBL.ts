...
const UserSavingBL = async (data: UserSavingAPIdataType) => {
  try {
    ...
    if (!data.summaryType || data.summaryType === "")
      throw new Error("Summary Type not found");
    if (
      !data.summaryType ||
      (data.summaryType !== "by_month" && data.summaryType !== "by_year")
    )
    ...
    if (
      userSavingResult.data.graph_members &&
      userSavingResult.data.graph_members.length !== 0
    ) {
    ...
    if (
      userSavingResult.data.progress_data &&
      userSavingResult.data.progress_data.length !== 0
    ) {
    ...
    const userSaving: UserSavingData = {
      lifeTimeSaving: userSavingResult.data.life_time_saving
        ? userSavingResult.data.life_time_saving.toFixed(2)
        : 0,
      currentYear: userSavingResult.data.current_year
        ? userSavingResult.data.current_year
        : 0,
      currentMonth: userSavingResult.data.current_month
        ? userSavingResult.data.current_month
        : 0,
